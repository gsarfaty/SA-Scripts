...
final_test<-final %>% 
  ...
  mutate(partner_lookback=case_when(
    ...
    DSPID=="70289_kz uMgungundlovu District Municipality" ~ "Health Systems Trust",
    TRUE ~ Partner),
    mech_id_lookback=case_when(
      ...
      DSPID=="70289_kz uMgungundlovu District Municipality" ~ "18481",
      TRUE ~ mech_code),
   agency_lookback=case_when(
      ...
      DSPID=="70289_kz uMgungundlovu District Municipality" ~ "HHS/CDC",
      TRUE ~ fundingagency
    ),
